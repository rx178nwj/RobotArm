...
function tableRows(d: DeviceSnapshot) {
  return d.channels.map(c => { const [state,cls] = channelState(c); return `<tr class="${cls === "bad" ? "row-fault" : cls === "warn" ? "row-disabled" : ""}"><td>CH${c.channel} · ${state}</td><td>${+c.present}</td><td>${+c.enable}</td><td>${+c.ok}</td><td>${c.degrees?.toFixed(3) ?? "invalid"}</td><td>${c.agc}</td><td>0x${c.magnetRaw.toString(16).padStart(2,"0")}</td><td>${c.readOk}</td><td>${c.readErr}</td></tr>`; }).join("");
}
