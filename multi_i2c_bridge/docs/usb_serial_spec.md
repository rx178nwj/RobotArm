...
### 4.0 `identity [set <16hex>]`

MonitorアプリがUSB再接続後に同一基板を識別するため、RP2040の基板固有IDを返す。

```
IDENTITY product=multi_i2c_bridge id=4250305639383219 protocol=1
```

`id`はEEPROMエミュレーション領域に保存した16桁の16進文字列であり、COMポート名やUSB接続位置、通常のUF2ファームウェア更新では変化しない。未設定時は外付けフラッシュの一意IDを初期値として使用する。

同一IDを返すフラッシュ部品が複数存在する場合、製造・初期設定時に`identity set <16hex>`で基板ごとに異なるIDを設定する。設定成功時は`OK identity id=<16hex>`、形式不正または保存失敗時は`ERR ...`を返す。
